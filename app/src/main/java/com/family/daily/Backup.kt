...
import com.family.daily.data.WorkSchedule
import com.family.daily.data.Template
...
object Backup {
    suspend fun export(db: AppDb): String {
        ...
        root.put("school", JSONArray().also { a -> db.school().all().first().forEach { s -> a.put(JSONObject().put("id", s.id).put("childId", s.childId).put("start", s.start).put("end", s.end).put("enabled", s.enabled).put("days", s.days).put("title", s.title).put("categoryId", s.categoryId).put("dayTimes", s.dayTimes)) } })
    ...
    suspend fun import(db: AppDb, json: String) {
        ...
        for (i in 0 until sc.length()) { val o = sc.getJSONObject(i); db.school().insert(com.family.daily.data.SchoolSchedule(id = o.optLong("id"), childId = o.optLong("childId"), start = o.optString("start", "08:00"), end = o.optString("end", "13:00"), enabled = o.optBoolean("enabled", true), days = o.optString("days", "2,3,4,5,6"), title = o.optString("title", "Школа"), categoryId = o.optLong("categoryId", 3), dayTimes = o.optString("dayTimes", ""))) }
        ...
        for (i in 0 until tp.length()) { val o = tp.getJSONObject(i); db.templates().insert(Template(title = o.optString("title"), dayOfWeek = o.optInt("dayOfWeek"), start = o.optString("start"), end = o.optString("end"), categoryId = o.optLong("categoryId", 1), silent = o.optBoolean("silent", true))) }
    }
}
