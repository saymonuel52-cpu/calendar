...
class Form(private val ctx: Context) {
    ...
    fun add(v: android.view.View) { root.addView(v, lp) }
                ...
                scope.launch {
                    val doSave: suspend () -> Unit = {
                        val id = if (existing != null) {
                            db.events().update(existing.copy(title = t, date = date.value, start = s, end = e2, allDay = allDay.isChecked, categoryId = catId, note = note.text.toString(), reminders = remVal)); existing.id
                        } else db.events().insert(Event(title = t, date = date.value, start = s, end = e2, allDay = allDay.isChecked, categoryId = catId, note = note.text.toString(), reminders = remVal))
                        db.participants().clear(id)
                        partIds.forEach { db.participants().insert(EventParticipant(id, it)) }
                        if (remVal.isNotBlank()) ReminderScheduler.scheduleFor(db, "EVENT", id, t, date.value, if (allDay.isChecked) "" else s, remVal)
                        onSaved?.invoke()
                    }
